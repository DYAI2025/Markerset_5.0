...
# Angepasst für Lean-Deep 3.2: Logging + LD-Metadaten
...
def connect_pinecone():
    ...
    logger.info(f"Pinecone verbunden mit: {environment}")
    return pc
...
def get_or_create_index(pc, index_name="hive-core", dimension=1536, region=None, drop_old=False):
    ...
    if index_name in existing and drop_old:
        ...
        logger.info(f"→ Alter Index gelöscht: {index_name}")
    if index_name not in existing:
        try:
            ...
            logger.info(f"→ Neuer Index erstellt: {index_name}")
        except Exception as e:
            if region != "us-west1":
                logger.warning(f"Region fehlerhaft, Fallback auf us-west1: {e}")
                pc.create_index(
                    name=index_name,
                    dimension=dimension,
                    metric="cosine",
                ...
                logger.info(f"→ Index mit Fallback erstellt: {index_name}")
            else:
                raise
    else:
        logger.info(f"→ Index gefunden: {index_name}")
...
def upsert_text(index, text, metadata=None, id=None):
    ...
    if id is None:
        id = str(hash(text))
    # Lean-Deep-Metadaten ergänzen
    full_metadata = dict(metadata or {})
    full_metadata["lean_deep_version"] = "3.4"
    index.upsert([
        {
            "id": id,
            "values": vector,
...
if __name__ == "__main__":
    ...
    logger.info(f"→ Embedding & Abfrage zu: {beispieltext}")
    ergebnis = query_similar(index, beispieltext)
    for match in ergebnis.matches:
        logger.info(f"{match.score:.3f} → {match.metadata}")
